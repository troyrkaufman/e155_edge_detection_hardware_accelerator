...
uint8_t *regcnfgr = regBuf;     // Points at current register address in camera
                                // module to access pixel data

// DMA2 Channels 2 and 3
uint8_t *currentBufferT =
    txBuffer1; // ACtive DMA buffer receiving processed data
uint8_t *transmitBufferT =
    NULL; // Buffer whose data is read to be sent over SPI
...
void processData(void) {
  if (bufferFullR && processBufferR) {
    ...
    uint8_t *outputBuffer =
        (currentBufferT == txBuffer1) ? txBuffer2 : txBuffer1;

    for (int i = 0; i < BUFFER_SIZE_R; i += PIXEL_BYTES) {
      uint16_t pixel =
          (processBufferR[i] << 8) |
          processBufferR[i + 1]; // Combine bytes to get RGB565 pixel
      outputBuffer[i / PIXEL_BYTES] =
          grayscaleConversion(pixel); // Convert to grayscale and store
    }
...
int main(void) {
  ...
  // pinMode(PA7, GPIO_ALT);    // MOSI We'll use PA7 in reality but for now
  // we'll use PB
...
void DMA1_Channel2_IRQHandler(void) {
  ...
  // SPI1->CR1 |= SPI_CR1_SPE; // Re-enable SPI before starting the next
  // transfer
...
/*   // Reset DMA2 Channel 2
    ...
    DMA2_Channel2->CCR  |= (_VAL2FLD(DMA_CCR_PL,0b11) |   // Priority is set to
   high _VAL2FLD(DMA_CCR_MINC, 0b1) | // memory address updates after every
   transmission _VAL2FLD(DMA_CCR_CIRC, 0b1) | // DMA NBYTE count will update to
   declared value _VAL2FLD(DMA_CCR_DIR, 0b1) |  // Peripheral to memory transfer
   ...
   length _VAL2FLD(DMA_CCR_PSIZE, 0b00)   // Set to byte length
    ...
    DMA2_Channel2->CNDTR |= _VAL2FLD(DMA_CNDTR_NDT, 1); // # pix per row * pixel
   width * # rows : 640 * 2 * 3
...
/*
   ...
   pixel width * # rows : 640 * 2 * 3
    ...
    DMA1_Channel3->CCR  |= (_VAL2FLD(DMA_CCR_PL,0b10) |   // Priority is set to
   medium _VAL2FLD(DMA_CCR_MINC, 0b1) | // memory address updates after every
   reception _VAL2FLD(DMA_CCR_CIRC, 0b1) | // DMA NBYTE count will update to
   declared value _VAL2FLD(DMA_CCR_DIR, 0b1) |  // Memory to peripheral transfer
   CHANGE BACK TO 1 _VAL2FLD(DMA_CCR_MSIZE, 0b00) | // Set to byte length
                            _VAL2FLD(DMA_CCR_PSIZE, 0b00)   // Set to byte
   length
                            );
    ...
    // Set DMA data transfer length (# of samples)
    DMA1_Channel3->CNDTR |= _VAL2FLD(DMA_CNDTR_NDT, 40); // # pix per row *
   pixel width * # rows : 640 * 2 * 3

    // Select the 1st option for mux to channel 3
    DMA1_CSELR->CSELR |= _VAL2FLD(DMA_CSELR_C2S, 0b0001);

    // Enable interrupt bit for channel 3 -----------------Is an interrupt
   needed? DMA1_Channel3->CCR |=  _VAL2FLD(DMA_CCR_TCIE, 1);
