...
USART_TypeDef *initUSART(int USART_ID, int baud_rate) {
  ...
  case USART1_ID:
    ...
    RCC->CCIPR |= (0b10 << RCC_CCIPR_USART1SEL_Pos); // Set HSI16 (16 MHz) as
                                                     // USART clock source

    GPIOA->AFR[1] |=
        (0b111 << GPIO_AFRH_AFSEL9_Pos) | (0b111 << GPIO_AFRH_AFSEL10_Pos);
  ...
  case USART2_ID:
    ...
    RCC->CCIPR |= (0b10 << RCC_CCIPR_USART2SEL_Pos); // Set HSI16 (16 MHz) as
                                                     // USART clock source
  ...
  // Set M = 00
  USART->CR1 &= ~(USART_CR1_M0 | USART_CR1_M1); // M=00 corresponds to 1 start
                                                // bit, 8 data bits, n stop bits
  ...
  USART->CR1 |=
      USART_CR1_TE | USART_CR1_RE; // Enable transmission and reception
